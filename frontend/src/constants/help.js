...
export const FIELD_TIPS = {
  ...
  finance_export: {
    export_type: '导出数据类型：contracts=合同, payments=收款记录, invoices=发票数据, rd_expenses=研发费用台账。',
...
export const PAGE_TIPS = {
  ...
  finance_all: {
    title: '全部记录',
    description: '查看所有收支记录，支持按类型、状态、资金来源筛选。支出记录可一键归入研发费用台账。',
    tips: [
      '收入显示为绿色正数，支出显示为红色负数',
      '资金来源为公司账户的记录直接入账，垫付/借款需后续结算',
      '结算状态标识该笔款项是否已完成对账或报销',
      '支出记录操作列显示「归入研发」按钮，点击可导入到研发费用台账',
      '已归入研发的支出记录显示「已入研」标签，不可重复导入',
      '研发费用台账中解绑后，原财务记录恢复「归入研发」按钮',
    ],
  },
  invoice_ledger: {
    title: '发票台账',
  ...
  dashboard: {
    title: '首页仪表盘',
    description: '仪表盘汇总当前经营状态，数据来源于各模块的关键业务事件（合同确认、收款录入、交付完成等），不实时计算，点击"手动刷新"可重新汇总最新数据。',
    tips: [
      '指标卡片点击可跳转到对应详情页面',
      '营收趋势图展示近 12 个月的收支对比',
      '现金流预测展示未来 90 天的净额趋势',
  ...
  minutes_page: {
    title: '会议纪要',
    description: '会议纪要记录每次客户或项目沟通的关键结论，必须关联项目或客户，确保信息可追溯、不丢失。',
    tips: [
      '纪要标题建议包含日期和主题，例如"2024-06-01 项目启动会"',
      '参与人员多人以逗号分隔',
      '结论避免模糊描述，应写明本次沟通达成的明确结论',
      '待办事项建议注明负责人和截止时间',
      '风险点记录沟通中识别到的风险或不确定因素',
      '关联项目和关联客户至少选一个',
      '每次保存纪要后系统自动留存版本快照，点击"版本对比"可选择任意两个版本进行对比',
    ],
  },
  tool_entries_page: {
    title: '工具入口台账',
    description: '工具入口台账记录当前每类业务动作使用的外部工具及其完成状态，帮助追踪哪些动作已完成、哪些结果已回填到系统。',
    tips: [
      '业务动作名称例如"合同电子签"、"发票开具"、"会议记录整理"',
      '当前执行该动作所使用的外部工具，例如"腾讯电子签"、"企业微信"',
      '动作状态：待处理→进行中→已完成→已回填',
      '已回填标记该动作的结果是否已回填到数据标云管对应模块',
    ],
  },
  leads_page: {
    title: '客户线索台账',
    description: '客户线索台账追踪潜在客户的跟进状态，不替代客户档案，仅记录正式成交前的轻量跟进信息。线索转化后可关联至客户模块。',
    tips: [
      '线索来源渠道例如"朋友介绍"、"平台主动询价"、"展会"',
      '跟进状态：初步接触→意向确认→已转化（终态）；无效为独立终态',
      '下一步跟进动作建议具体描述，例如"本周五发送方案报价"',
      '可选关联已有客户档案（线索转化后使用）',
      '可选关联已有项目（线索对应具体项目时使用）',
    ],
  },
  rd_expense: {
    title: '研发费用台账',
    description: '按税法六大类归集研发费用，用于科技型中小企业认定和研发费用加计扣除申报。支持与财务支出记录关联导入，也可独立录入。',
    tips: [
      '费用大类必须选择，子分类建议尽量填写以便后续统计',
      '每笔费用自动生成编号（RD-YYYYMMDD-序号），便于追溯',
      '费用发生日期决定会计期间归属，影响年度/季度汇总',
      '审核流程：草稿 → 已核实 → 已提交申报。草稿可删除，提交后不可修改',
      '支持导出 Excel（含汇总表 + 明细表），格式符合申报要求',
      '「全部记录」中支出类型可点击「归入研发」一键导入，金额和日期自动填充',
      '财务导入的记录显示「财务导入」标签，可「解绑」恢复为独立记录',
      '解绑后原财务记录可重新归入研发，研发费用台账中该记录变为「手工录入」',
      '按年份/季度/大类/状态多维度筛选，顶部显示当前筛选条件的汇总数据',
      '六大类：人员人工、直接投入、折旧、无形资产摊销、设计试验其他、委托外部研发',
      '仪表盘「研发费用」卡片按年度汇总，点击可跳转到研发费用台账',
    ],
  },
}
...
FIELD_TIPS.minutes = {
  title: '纪要标题，建议包含日期和主题，例如"2024-06-01 项目启动会"。',
  participants: '参与人员，多人以逗号分隔。',
  conclusions: '本次沟通达成的明确结论，避免模糊描述。',
  action_items: '后续需跟进的具体行动项，建议注明负责人和截止时间。',
  risk_points: '沟通中识别到的风险或不确定因素。',
  project_id: '关联项目（与客户至少选一个）。',
  client_id: '关联客户（与项目至少选一个）。',
}

FIELD_TIPS.tool_entry = {
  action_name: '业务动作名称，例如"合同电子签"、"发票开具"、"会议记录整理"。',
  tool_name: '当前执行该动作所使用的外部工具，例如"腾讯电子签"、"企业微信"。',
  status: '动作当前状态。待处理：尚未开始；进行中：已发起但未完成；已完成：动作本身已完成；已回填：结果已录入系统。',
  is_backfilled: '标记该动作的结果是否已回填到数据标云管对应模块。',
}

FIELD_TIPS.lead = {
  source: '线索来源渠道，例如"朋友介绍"、"平台主动询价"、"展会"。',
  status: '跟进状态。初步接触：已有初步沟通；意向确认：客户明确有采购意向；已转化：已成为正式客户；无效：确认不会成交。',
  next_action: '下一步跟进动作，建议具体描述，例如"本周五发送方案报价"。',
  client_id: '可选关联已有客户档案（线索转化后使用）。',
  project_id: '可选关联已有项目（线索对应具体项目时使用）。',
}

FIELD_TIPS.rd_expense = {
  rd_category: '费用大类——按税法六大类归集：人员人工、直接投入、折旧、无形资产摊销、设计试验其他、委托外部研发。选择大类后子分类会自动过滤。',
  rd_sub_category: '费用子分类——大类的细粒度拆分。如「人员人工」下有工资薪金、五险一金等。选填项，但建议尽量填写以便后续统计。',
  amount: '费用金额（不含税），必须大于 0。系统自动计算价税合计 = 金额 + 税额。',
  tax_amount: '该笔费用的税额。如取得增值税专用发票可填写进项税额，无发票则填 0。',
  expense_date: '费用实际发生日期，用于会计期间归属（自动计算 YYYY-MM）和年度/季度汇总筛选。',
  project_id: '关联的研发项目。选填，但建议关联以便按项目维度统计研发投入。',
  vendor_name: '供应商或收款方名称。如为工资薪金则可填公司名称或"内部"。',
  invoice_no: '对应的凭证号或发票号。有正式凭证时必填，便于审计追溯。',
  has_invoice: '是否已取得有效凭证（发票/收据/银行回单）。影响加计扣除的合规性判断。',
  status: '审核状态：草稿→已核实→已提交申报。草稿状态可删除和编辑，提交后不可修改。',
}
...
export const CORE_CONCEPTS = [
  ...
  {
    term: '研发费用台账（RdExpense）',
    definition: '按税法六大类归集的研发费用独立台账，用于科技型中小企业认定和研发费用加计扣除申报。与现有财务记录(FinanceRecord)松耦合——可关联也可独立录入，不影响原有业务流程。',
    related: ['项目', '收款记录(FinanceRecord)', '合同'],
    key_rule: '仅草稿(draft)状态的记录可删除和编辑；已核实(verified)后可提交(submitted)；提交后不可修改。一个 FinanceRecord 可对应多条 RdExpense（如一张发票含多种研发费用类别）',
  },
]
