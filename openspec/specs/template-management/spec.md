...
系统 SHALL 存储 Jinja2 模板用于文档生成，模板类型白名单扩展为包含报告类型。
...
- **THEN** 默认模板 SHALL 存在：quotation、contract、report_project、report_customer 各一个
- **THEN** 每个默认模板 SHALL 有 is_default = 1
- **THEN** 每个默认模板 SHALL 有 unique constraint on (template_type, is_default) WHERE is_default = 1

#### Scenario: Template type whitelist includes report types
- **WHEN** 查看 TEMPLATE_TYPE_WHITELIST
- **THEN** 包含 "quotation", "contract", "report_project", "report_customer"

#### Scenario: Report templates idempotent insert
- **WHEN** 系统初始化且 report_project / report_customer 默认模板已存在
